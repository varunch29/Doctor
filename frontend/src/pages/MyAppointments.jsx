...
const MyAppointments = () => {
  ...
  return (
    <div>
              ...
              {!item.cancel && (
                <button
                  onClick={() => {
                    ...
                    const roomId = generateChatRoomId(userData?._id, item.docData._id);
                    setOpenChatId(roomId);
                    setChatDoctor(item.docData.name);
                    setChatDoctorId(item.docData._id);
                  }}
                  className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300'
      ...
      {/* Chat Popup - now uses consistent room ID */}
      {openChatId && (
        <ChatPopup
          appointmentId={openChatId}        // Now this is the consistent room ID
          userId={userData?._id}            // patient/user ID
          doctorId={chatDoctorId}          // doctor ID
          doctorName={chatDoctor}          // doctor name
          onClose={() => {
            setOpenChatId(null);
            setChatDoctor('');
